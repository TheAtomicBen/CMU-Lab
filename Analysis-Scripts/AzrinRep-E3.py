...
def wrangler(inname, inpath, outpath):
    ...
    #BO Right responses
    count1 = 0
    for i, (j, k) in izip(ba50_c.event, enumerate(borpointer2)):
        if i == 400:
            count1 += 1
            borpointer2[j] = count1
        if i == 410:
            count1 = 0
            borpointer2[j] = count1
    ...
    #FR Value
    count = 0
    for (i, j), (k, l), m in izip(enumerate(trial2), enumerate(frval), ratio):
        if m > 25:
            frval[k] = j*25
        else:
            frval[k] = 25
    ...
    ba50_c = ba50_c.assign(frval=frval)
    ba50_c = ba50_c.assign(borpointer2=borpointer2)
    ...
    ba50_c = ba50_c[['date', 'subject', 'frval', 'runnum', 'trial', 'trial2', 'frpointer', 'frpointerB', 'borpointer', 'borpointer2', 'bolpointer', 'time', 'fr_irt', 'prp', 'fr_prp', 'uo_prp', 'event']]
...
for rat in rats:
    ...
    wrangler(rat, RAW + '!_Subject_{}_Experiment_FR_Unobserving_E3.Group_1' .format(rat), RF + rat + '.csv')
